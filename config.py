...
class Config:
    ...
    WEEKLY_BACKUP_DAY = os.getenv('WEEKLY_BACKUP_DAY', 'sunday')
    CONTRACT_ALERT_DAYS = [int(d) for d in os.getenv('CONTRACT_ALERT_DAYS', '90,60,30').split(',')]
    ...
    def validate(cls):
        ...
        if not cls.GOOGLE_CREDENTIALS_PATH.exists():
            errors.append(f"❌ Arquivo de credenciais não encontrado: {cls.GOOGLE_CREDENTIALS_PATH}")
