...
def run_command(cmd: list, description: str) -> bool:
    ...
    try:
        result = subprocess.run(
            cmd, check=True, capture_output=True, text=True, cwd=Config.BASE_DIR
        )
        if result.stdout:
...
def main():
    ...
    parser.add_argument(
        "--skip-photos", action="store_true", help="Pula download de fotos"
    )
    ...
    if not args.skip_photos:
        steps.append(
            (
                [
                    sys.executable,
                    "scripts/maintenance/download_photos.py",
                    "--missing-only",
                ],
