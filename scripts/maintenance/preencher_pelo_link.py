...
def extrair_dados_tm(url):
    ...
    try:
        ...
        def buscar_info(soup, padroes):
            for info_box in soup.find_all(
                "div", class_=["info-table", "info-table--right-space"]
            ):
                for label in info_box.find_all(
                    "span", class_="info-table__content--regular"
                ):
        ...
        # Nacionalidade
        try:
            nacionalidade_label = soup.find(
                "span", string=re.compile(r"Nacionalidade|Citizenship")
            )
...
def main():
    ...
    for i in range(1, max_len):
        ...
        if "transfermarkt" in str(link) and (
            not nome_atual or nome_atual.strip() == ""
        ):
            ...
            if dados:
                print(
                    f"   ✅ Encontrado: {dados['nome']} ({dados['posicao']} - {dados['idade']} anos)"
                )

                updates = [
                    {"range": f"B{linha_excel}", "values": [[dados["nome"]]]},  # Nome
                    {
                        "range": f"C{linha_excel}",
                        "values": [[dados["posicao"]]],
                    },  # Posição
                    {
                        "range": f"D{linha_excel}",
                        "values": [[dados["nacionalidade"]]],
                    },  # Nacionalidade
                    {
                        "range": f"E{linha_excel}",
                        "values": [[dados["idade"]]],
                    },  # Idade (Corrigida)
                    ...
                    {
                        "range": f"I{linha_excel}",
                        "values": [[dados["fim_contrato"]]],
                    },  # Contrato
                    # --- CORREÇÃO COLUNAS ---
                    {
                        "range": f"J{linha_excel}",
                        "values": [[dados["altura"]]],
                    },  # Altura (Coluna J)
                    {
                        "range": f"K{linha_excel}",
                        "values": [[dados["pe"]]],
                    },  # Pé (Coluna K)
                ]
    ...
    if contador == 0:
        ...
        print(
            "   Dica: Cole links na coluna 'TM' (Coluna O) e deixe a coluna 'Nome' (Coluna B) vazia."
        )
    else:
