...
def restaurar_backup(arquivo_backup):
    ...
    try:
        ...
        if os.path.exists('scouting.db'):
            backup_atual = f'backups/antes_restaurar_{datetime.now().strftime("%Y%m%d_%H%M%S")}.db'
...
def menu():
    ...
    while True:
        ...
        if opcao == "1":
            ...
            listar_backups()
            arquivo = input("\nDigite o nome completo do arquivo (ex: scouting_backup_20250121_143022.db): ").strip()
            if arquivo:
